...
public:
    ...
    vec3 crumple(const vec2& tc, float t) const
    {
        float amp = 1.0f;
        float freq = 1.0f;
        float value = 0.0f;
...
#else
    Texture* texCrumple = scene.take(new TextureCellNoise(worleyTex));
    Texture* normalMap = scene.take(bumpMapToNormalMap(texCrumple, 16.0f, 256, 256));
    Texture* texKd = scene.take(new TextureConstant(vec3(0.5f)));
    Texture* texKs = scene.take(new TextureConstant(vec3(0.0f)));
    Texture* texS = scene.take(new TextureConstant(vec3(120.0f)));
    Material* boxMat = scene.take(new MaterialPhong(texKd, texKs, texS, nullptr, normalMap));
#endif
    Transformation boxT;
