...
vec3 pathSample(const Scene& scene, const Ray& startRay, Prng& prng)
{
    ...
    for (int segment = 0; segment < MaxPathSegments; segment++) {
        ...
        // scatter the ray at the hit point towards the new direction
        ScatterRecord sr = hr.material->scatterToDirection(ray, hr, lightDir);
        ...
        throughput /= lightDirP;
        // update the ray for the next path segment
        ray = Ray(hr.position, lightDir, ray.time);
