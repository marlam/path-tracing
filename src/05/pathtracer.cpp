...
HitRecord hit(const Scene& scene, const Ray& ray)
{
    ...
    for (size_t i = 0; i < scene.surfaces.size(); i++) {
        HitRecord tmpHr = scene.surfaces[i]->hit(ray, amin, amax);
...
int main(void)
{
    ...
    if (!importIntoScene(scene, "CornellBox-Original.obj"))
        return 1;
    fprintf(stderr, "Scene contains %u surfaces\n", unsigned(scene.surfaces.size()));
